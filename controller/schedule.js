...
// function process_schedule(cat_id, date, time, sched_id, interval){
...
function process_schedule(cat_id, time, sched_id, interval) {
    let query = `
        INSERT INTO messages (lead_ref, sched_ref, mess_date, mess_time, mess_status)
        SELECT lead_id, ?, DATE_ADD(ref_date, INTERVAL ? DAY), ?, 0
        FROM leads
        WHERE category_ref = ? AND status = 1
    `;
    con.query(query, [sched_id, interval, time, cat_id], (err, result) => {
        if (err) throw err;
        console.log("messages scheduled");
    });
}
